...
export function AddUserAchivment(res, error) {
    ...
    let settingsSer= JSON.stringify(settings);
    let settingsKey = MD5(settingsSer);
    var userAchivment = LoadUserAchivment(userName, settingsKey);//load prev Achivment
    userAchivment.push(res+'/'+ error)
    const userAchivmentStr = JSON.stringify(userAchivment)
    localStorage.setItem(userName + "_" + settingsKey, userAchivmentStr);
}
